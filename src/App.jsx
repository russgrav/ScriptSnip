...
function App() {
  ...
  const handleFiles = useCallback((files) => {
    ...
    if (validation.videoFiles.length > 0 && condenserEngineRef.current) {
      const estimate = condenserEngineRef.current.estimateMemoryUsage(validation.videoFiles);
  ...
  const startProcessing = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('Failed to load FFmpeg')) {
        ...
        errorMessage = 'No matching subtitle file found. Make sure your subtitle file has the same name as your video file.';
      }
      ...
      <div className="file-list">
        ...
        {validation.invalidFiles.length > 0 && (
          <div className="warning-message">
            <AlertCircle size={16} />
            {validation.invalidFiles.length} unsupported file(s) will be ignored.
          </div>
        )}
      </div>
    );
  ...
  const renderProcessTab = () => (
    <div className="card">
      <div className="card-content">
        ...
        {showProgress && (
          <div className="progress-section">
            <div className="progress-text">
              {progress.message || `${progress.stage}: ${progress.progress}/${progress.total}`}
            </div>
        ...
        <button
          className={`btn ${isProcessing ? 'btn-danger' : selectedFiles.filter(f => fileManagerRef.current.isVideoFile(f)).length > 0 ? 'btn-success' : 'btn-secondary'}`}
          onClick={startProcessing}
          disabled={!isProcessing && selectedFiles.filter(f => fileManagerRef.current.isVideoFile(f)).length === 0}
        ...
        >
          ...
          ) : selectedFiles.filter(f => fileManagerRef.current.isVideoFile(f)).length > 0 ? (
            <>
              <Scissors size={16} />
              Process {selectedFiles.filter(f => fileManagerRef.current.isVideoFile(f)).length} video file(s)
            </>
          ) : (
            <>
              <Upload size={16} />
              ...
              return (
                <div key={index} className="result-item">
                  <div className="result-info">
                    <h4>{fileManagerRef.current.generateOutputFilename(result.originalFilename, config.outputFormat)}</h4>
                    <div className="result-stats">
                      Original: {fileManagerRef.current.formatDuration(result.originalDuration)} → 
                      Condensed: {fileManagerRef.current.formatDuration(result.condensedDuration)} 
        ...
        <div className="card-content">
          <div className="config-grid">
            <div className="form-group">
              <label className="form-label" htmlFor="padding">Padding (ms)</label>
              <input
                type="number"
                id="padding"
                ...
                onChange={(e) => setConfig(prev => ({ ...prev, padding: parseInt(e.target.value) || 500 }))}
              />
            </div>
            <div className="form-group">
          ...
          <div className="config-grid">
            <div className="form-checkbox">
              <input
                ...
                onChange={(e) => setConfig(prev => ({ ...prev, filterParentheses: e.target.checked }))}
              />
              <label htmlFor="filter-parentheses">Filter text in parentheses/brackets</label>
            </div>
            <div className="form-group">
              <label className="form-label" htmlFor="filtered-characters">Filtered Characters</label>
              <input
                type="text"
                id="filtered-characters"
                className="form-input"
                value={config.filteredCharacters || '♩♪♫♬〜〜'}
                onChange={(e) => setConfig(prev => ({ ...prev, filteredCharacters: e.target.value }))}
                placeholder="Characters to filter out"
              />
            </div>
            <div className="form-group">
              <label className="form-label" htmlFor="sub-suffix">Subtitle Suffix</label>
              <input
                type="text"
                id="sub-suffix"
                className="form-input"
                value={config.subSuffix || ''}
                onChange={(e) => setConfig(prev => ({ ...prev, subSuffix: e.target.value }))}
          ...
          <div className="config-grid">
            <div className="form-checkbox">
              <input
                ...
                onChange={(e) => setConfig(prev => ({ ...prev, outputCondensedSubtitles: e.target.checked }))}
              />
              <label htmlFor="output-condensed-subtitles">Output condensed subtitles</label>
            </div>
            <div className="form-group">
              <label className="form-label" htmlFor="condensed-subtitles-format">Subtitle Format</label>
              <select
                id="condensed-subtitles-format"
                className="form-select"
                value={config.condensedSubtitlesFormat || 'srt'}
                onChange={(e) => setConfig(prev => ({ ...prev, condensedSubtitlesFormat: e.target.value }))}
              >
    ...
    <div className="card">
      ...
      <div className="card-content">
        <p style={{ marginBottom: '2rem', fontSize: '1.1rem', lineHeight: '1.7' }}>
          ScriptSnip extracts speech audio from video files based on subtitle timing, creating condensed audio perfect for language immersion learning.
        </p>
        
        <h4 style={{ marginBottom: '1rem', color: 'var(--text-primary)' }}>How it works:</h4>
        <ol style={{ marginBottom: '2rem', paddingLeft: '1.5rem', lineHeight: '1.7' }}>
        ...
        <h4 style={{ marginBottom: '1rem', color: 'var(--text-primary)' }}>Supported formats:</h4>
        <div style={{ marginBottom: '2rem', lineHeight: '1.7' }}>
          <p><strong>Video:</strong> MP4, MKV, AVI, MOV, FLV, WebM, MP3, WAV, FLAC, M4A, AAC</p>
          <p><strong>Subtitles:</strong> SRT, ASS, SSA, VTT</p>
          <p><strong>Output:</strong> MP3, FLAC, WAV, AAC, OGG</p>
        </div>

        <h4 style={{ marginBottom: '1rem', color: 'var(--text-primary)' }}>Privacy:</h4>
